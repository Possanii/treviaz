...
export class CreateServiceOwnerService {
    async execute(data: Omit<IServiceOwner, 'id'> & { password: string }): Promise<IServiceOwner> {
        const existingServiceOwner = await prisma.serviceOwner.findUnique({
            where: { email: data.email }
        })

        if (existingServiceOwner) {
            throw new BadRequestError('serviceOwner', 'service owner with this email already exists')
        }
